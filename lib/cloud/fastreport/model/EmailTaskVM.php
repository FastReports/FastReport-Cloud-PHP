 ...
 */
class EmailTaskVM implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setName($name)
    {
        if (!is_null($name) && (mb_strlen($name) > 50)) {
            throw new \InvalidArgumentException('invalid length for $name when calling EmailTaskVM., must be smaller than or equal to 50.');
        }
        if (!is_null($name) && (mb_strlen($name) < 1)) {
            throw new \InvalidArgumentException('invalid length for $name when calling EmailTaskVM., must be bigger than or equal to 1.');
        }
    ...
    public function setSubscriptionId($subscription_id)
    {
        ...
        if (!is_null($subscription_id) && (!preg_match("/(^$)|(^[A-Fa-f0-9]{24}$)/", $subscription_id))) {
            throw new \InvalidArgumentException("invalid value for $subscription_id when calling EmailTaskVM., must conform to the pattern /(^$)|(^[A-Fa-f0-9]{24}$)/.");
        }
